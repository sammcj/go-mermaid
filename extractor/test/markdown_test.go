...
func TestExtractFromMarkdown_UnclosedBlock(t *testing.T) {
	...
	if len(blocks) != 1 {
		t.Fatalf("expected 1 block (unclosed block should be handled), got %d", len(blocks))
	}

	if blocks[0].DiagramType != "flowchart" {
		t.Errorf("expected type 'flowchart', got %q", blocks[0].DiagramType)
	}

	if blocks[0].Source != "flowchart TD\n    A --> B" {
		t.Errorf("unexpected source: %q", blocks[0].Source)
	}
}

func TestExtractFromMarkdown_UnclosedEmptyBlock(t *testing.T) {
	// Test that unclosed empty blocks at EOF are not included
	markdown := "```mermaid\n   \n  "
	...
	if len(blocks) != 0 {
		t.Errorf("expected 0 blocks (unclosed empty block should be skipped), got %d", len(blocks))
	}
}

func TestExtractFromMarkdown_EndLineTracking(t *testing.T) {
	markdown := `Start
` + "```mermaid" + `
flowchart TD
    A --> B
...
End`
	...
	// Check line tracking
	if blocks[0].LineOffset != 3 {
		t.Errorf("expected line offset 3, got %d", blocks[0].LineOffset)
	}

	if blocks[0].EndLine != 4 {
		t.Errorf("expected end line 4, got %d", blocks[0].EndLine)
	}
}

func TestExtractFromMarkdown_MixedContent(t *testing.T) {
	markdown := `# Title
