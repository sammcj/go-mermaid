...
func TestValidRelationshipReferencesRule(t *testing.T) {
	tests := []struct {
		...
		{
			name: "implicit source entity (valid)",
			diagram: &ast.ERDiagram{
				Entities: []ast.EREntity{
			...
			wantErr: false, // Entities can be implicitly defined in relationships
		},
		{
			name: "implicit target entity (valid)",
			diagram: &ast.ERDiagram{
				Entities: []ast.EREntity{
					{Name: "CUSTOMER", Pos: ast.Position{Line: 2, Column: 1}},
				},
		...
		{
			name: "both entities implicit (valid)",
			diagram: &ast.ERDiagram{
				Entities: []ast.EREntity{},
				Relationships: []ast.ERRelationship{
...
func TestValidateER(t *testing.T) {
	tests := []struct {
		...
		{
			name: "implicit entity in relationship (valid)",
			diagram: &ast.ERDiagram{
				Entities: []ast.EREntity{
					{Name: "CUSTOMER", Pos: ast.Position{Line: 2, Column: 1}},
				},
